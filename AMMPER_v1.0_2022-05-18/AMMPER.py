...
Copyright © 2023 United States Government as represented by the Administrator of the National Aeronautics
...
AND HOLD HARMLESS THE UNITED STATES GOVERNMENT, ITS CONTRACTORS AND SUBCONTRACTORS, AS WELL AS ANY PRIOR RECIPIENT,
TO THE EXTENT PERMITTED BY LAW.  RECIPIENT'S SOLE REMEDY FOR ANY SUCH MATTER SHALL BE THE IMMEDIATE, UNILATERAL
...
for g in range(1,gen+1):
    ...
    if radType == "150 MeV Proton":
        ...
        if g == radGen:
            ...
            for it in range(len(GCRSimFluenceDat)):
                ...
                radData_trans = genTraverse_groundTesting(N,protonEnergy,trackNum,energyThreshold)
                # compile radData from this traversal with all radData
                radData = np.vstack([radData,radData_trans])
    ...
    if (radType == "150 MeV Proton" and g == radGen and Gy != 0) or (radType == "Deep Space") or (radType == "GCRSim" and g == radGen):        
    ...
    if (radType == "150 MeV Proton" and g > radGen and Gy != 0 and cellType != "rad51") or (radType == "Deep Space" and cellType != "rad51") or (radType == "GCRSim" and g > radGen and cellType != "rad51"):
